...
object Lag extends java.io.Serializable {
  /**
  ...
  def lagMatTrimBoth(x: Array[Double], maxLag: Int, includeOriginal: Boolean)
    : Array[Array[Double]] = {
    val numObservations : Int = x.length
    val numRows : Int = numObservations - maxLag
    val numCols : Int = maxLag + (if (includeOriginal) 1 else 0)
    val lagMat  = Array.ofDim[Double](numRows, numCols)

    val initialLag : Int = if (includeOriginal) 0 else 1
  ...
  def lagMatTrimBoth(x: Vector, maxLag: Int, includeOriginal: Boolean): Matrix = {
    val numObservations : Int = x.size
    val numRows : Int = numObservations - maxLag
    val numCols : Int = maxLag + (if (includeOriginal) 1 else 0)
  ...
  def lagMatTrimBoth(
      ...
      colOffset: Int): Unit = {
    val numRows : Int = outputMat.numRows
    val numTruncatedRows : Int = x.size - numRows
